...
const getParticipantsCount = (users: IChatUser[]) => {
    ...
    if (
        userCount.toString().slice(-1) === '2' ||
        userCount.toString().slice(-1) === '3' ||
        userCount.toString().slice(-1) === '4'
    ) {
...
export const ChatInfoBlock = (
    ...
    new DivBlock({
        className:
            chats && activeChat && events ? 'chat-info' : 'chat-info empty',
        content: [
            new DivBlock({
                className:
                    chats && activeChat
                        ? 'chat-info__title'
                        : 'chat-info__text-empty',
                content:
                    chats && activeChat
                        ? new Text({
                              content: getChatName(chats, activeChat.id),
                          })
                        : new Skeleton({ height: 16, isAnimation: true }),
            }),
            new DivBlock({
                className: activeChat
                    ? 'chat-info__subtitle text-dark'
                    : 'chat-info__text-empty',
                content: activeChat
                    ? new Text({
